...
class DecisionTreeClassifier:
    ...
    def predict_proba(self, X):
        proba_preds = np.zeros(shape=(X.shape[0], len(self.classes)))
        for i, x in enumerate(X):
            proba_preds[i, :] = self._get_predict(
                self.tree['root'],
                x
...
class MyRandomForestClassifier:
    ...
    def __init__(self, n_estimators=None, max_depth=None, max_features=None, min_leaf_samples=None):
